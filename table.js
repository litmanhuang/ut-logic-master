...
const challengeIdsStored = localStorage.getItem("challengeIdsStoredLocal");
const challengeIdsStored2 = localStorage.getItem("challengeIdsStoredLocal2");
...
function populateProgressTable(data, tbody) {
  for (let i = 0; i < data.length; i++) {
    const { id, attendance, email, firstName, lastName, ...newObject } =
      data[i];
...
function generateAttendanceTable(students) {
  ...
  const uniqueDates = [
    ...new Set(
      students.flatMap((student) =>
        student.attendance.map((date) => date.split(":")[0])
      )
    ),
  ];
  ...
  const tableRows = students.map((student) => {
    ...
    uniqueDates.forEach((date) => {
      const attendanceData = student.attendance.find((item) =>
...
async function fetchAssignmentData(studentId, course, instructor, apiSecret) {
  ...
  try {
    const response = await fetch(
      `https://carnap.io/api/v1/instructors/${instructor}/courses/${course}/students/${studentId}/submissions`,
      requestOptions
    );
    const data = await response.json();
    return data;
...
function dateIsValid(accessDate, validDates, attendance) {
  ...
  return presenceDates.some(
    (date) =>
      date.presence &&
      accessDate >= new Date(date.startTime) &&
      accessDate <= new Date(date.endTime)
  );
}
...
async function findChallengeResult(
  data,
  attendance,
  validDatesInput,
  challengeIds
) {
  const challengeIdArray = challengeIds.reduce(
    (accumulator, currentObject) =>
      accumulator.concat(Object.values(currentObject)),
    []
  );
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (index !== -1) {
      if (
        correct !== null &&
        challengeData[index] === "no attempt" &&
        dateIsValid(accessDate, validDatesInput, attendance)
      ) {
...
async function catchCrime(
  ...
  let crimeReport = {
    ...
    firstName: firstName,
    lastName: lastName,
    reports: challengeIds.map((challenge) => ({
      [Object.keys(challenge)[0]]: "",
    })),
  };
  ...
  for (let i = 0; i < assignments.length; i++) {
    ...
    let challengeObject = challengeIds.find(
      (obj) => Object.values(obj)[0] === challenge
    );

    if (challengeObject) {
      let challengeName = Object.keys(challengeObject)[0];

      let isCrimeChallenge = !validDates.some(
        (date) =>
          accessDate >= new Date(date.startTime) &&
          accessDate <= new Date(date.endTime)
      );
      ...
      if (isCrimeChallenge) {
        ...
        crimeReport.reports[challengeIds.indexOf(challengeObject)][
          challengeName
        ] = crimeDate;
      } else {
        crimeReport.reports[challengeIds.indexOf(challengeObject)][
...
function generateCrimeReportTable(crimeReports) {
  const uniqueChallenges = [
    ...new Set(
      crimeReports.flatMap((report) =>
        report.reports.map((challenge) => Object.keys(challenge)[0])
      )
    ),
  ];
  ...
  const tableRows = crimeReports.map((crimeReport) => {
    ...
    uniqueChallenges.forEach((challenge) => {
      const challengeData = crimeReport.reports.find(
        (report) => Object.keys(report)[0] === challenge
      );
...
async function createStudentProgress(
  ...
  const challengeIdParsed = JSON.parse(challengeIdInput);
  console.log(challengeIdParsed);
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let assignmentData = await fetchAssignmentData(
        studentCarnapData.id,
        courseInput,
        instructor,
        apiKey
      );
      ...
      console.log(
        "assignment data:" +
          ...
          studentCarnapData.id +
          " name =" +
          studentCarnapData.firstName +
          " " +
          studentCarnapData.lastName
      );
      console.log(assignmentData);

      let attendance = await getStudentAttendence(
        studentCarnapData.id,
        attendanceParsed
      );
      console.log(attendance);

      let studentCrimeReport = await catchCrime(
        studentCarnapData.id,
        studentCarnapData.firstName,
        studentCarnapData.lastName,
        assignmentData,
        validDatesInputParsed,
        challengeIdParsed
      );
      crimeReports.push(studentCrimeReport);
      let challengeData = await findChallengeResult(
        assignmentData,
        attendance,
        validDatesInputParsed,
        challengeIdParsed
      ...
      let student = new studentCarnap(
        studentCarnapData.id,
        studentCarnapData.email,
        studentCarnapData.firstName,
        studentCarnapData.lastName,
        challengeData[0],
        challengeData[1],
